...
// ── GENERATE BRACKET OTOMATIS (admin) ─────────────────────────
// Tetap disimpan kalau nanti kamu mau pakai otomatis.
// Tapi alur utama sekarang pakai createManualMatch.
  ...
  try {
    ...
    if (teams.length < 2) {
      return res.status(400).json({
        message: "Minimal 2 tim terkonfirmasi untuk membuat bracket.",
      });
    }
    ...
    return res.status(201).json({
      message: "Bracket otomatis berhasil dibuat!",
...
// ── BUAT MATCH MANUAL (admin) ─────────────────────────────────
const createManualMatch = async (req, res) => {
  try {
    const { tournament_id, ronde, urutan, team1_id, team2_id, jadwal } =
      req.body;

    if (!tournament_id || !ronde || !urutan || !team1_id || !team2_id) {
      return res.status(400).json({
        message: "Data match belum lengkap.",
      });
    }
    ...
    if (!team1 || !team2) {
      return res.status(400).json({
        message:
          "Tim harus sudah terkonfirmasi dan berada di turnamen yang sama.",
      });
    }
    ...
    if (existingMatch) {
      return res.status(409).json({
        message: "Ronde dan urutan match ini sudah digunakan.",
      });
    }
    ...
    return res.status(201).json({
      message: "Match manual berhasil dibuat.",
      match,
    });
  } catch (err) {
